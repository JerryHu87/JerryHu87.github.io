...
vector<pair<int, int> > nodes[5001];int d1[5001];int d2[5001];
...
int main(){
	...
	while(!pq.empty()){
		...
		for(int i = 0;i<nodes[current.second].size();i++){
			if(d2[nodes[current.second][i].second] > d2[current.second]+nodes[current.second][i].first){
				d2[nodes[current.second][i].second] = d2[current.second]+nodes[current.second][i].first;
				pq.push(make_pair(d2[nodes[current.second][i].second],nodes[current.second][i].second));
			}
		}
	}
	...
	for(int i = 0;i<m;i++){
		...
		if(d1[edges[i].second.second]+d2[edges[i].second.first]+ edges[i].first< min2 && d1[edges[i].second.second]+d2[edges[i].second.first]+ edges[i].first != min)min2 = d1[edges[i].second.second]+d2[edges[i].second.first]+ edges[i].first;
	}
	printf("%d\n",min2);
	return 0;
}
